...
function playSequence() {
  ...
  const notesMap = piano.keyboard.notesMap;
  const sequence = getClickedNotes(notesMap);
  playNotesSequence(sequence);
}
function playNotesSequence(sequence) {
  sequence.reduce((promise, note) => {
    return promise.then(() => {
      return playNotePromise(note);
    });
  }, Promise.resolve());
}

function playNotePromise(note) {
